...
export default class NewGroup extends Component {
  ...
  validate = () => {
    ...
    if (!this.state.groupName) { return '请输入组名称' }
    if (!this.state.groupSize) { return '请输入组容量' }
    if (this.state.mobile.length !== 11) { return '手机号格式不正确' }
    if (this.state.groupSize > 200) { return '组最多支持200人' }
    return 200
  }
  signIn = () => {
    ...
    return (
      <View className='creat-group-page'>
        <View className='form-area'>
          ...
          <AtInput name='groupName' title='组名称' placeholder='请输入组名称' value={groupName} onChange={this.changegroupName} />
          <AtInput name='groupSize' title='组容量' type='digit' placeholder='请输入组最大人数' value={groupSize} onChange={this.changegroupSize}>
            <View className='right'>人</View>
          </AtInput>
        </View>
        <View className='sign-area'>
          <AtButton type='primary' onClick={this.signIn}>创建组</AtButton>
        </View>
        <AtModal isOpened={creatSuccess}>
          <AtModalHeader>创建成功</AtModalHeader>
          <AtModalContent className='modal-cnt'>
            <View>恭喜您，组创建成功</View>
            <View style={{ textAlign: 'left', paddingLeft: 40}} >　组号：{newGroupInfo.groupId}　　
            ...
            <View style={{ textAlign: 'left', paddingLeft: 40}}>组名称：{this.state.groupName}</View>
          </AtModalContent>
